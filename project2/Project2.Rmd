---
title: "Project 2"
author: "<AT>510603294,  520438936,  510513719,  520450156,  rachi </AT>"
date: "University of Sydney | `r Sys.Date()`"
output:
  html_document:
    code_folding: hide
    footer: Produced by.   Andrea, Jia, Sacha, Rachi & Isaac
  pdf_document: default
subtitle: Which factors influence a student's grade?
---
```{css newstyles, echo=FALSE}
h3{
margin-top : 16px;
}
h1, h2 { /* add border to h1 and h2 */
  border-bottom: solid 1px #666;
  margin-top : 36px;
}
body { /* Make main text colour black */
  color: black;
  background-image : url("data:image/jpeg;base64,");
  background-repeat : no-repeat;
  background-size: cover;
}
a { /* Link colours */
  color: blue;
}
AT{
 font-size: 90%;
}
```
## Executive Summary
<y class="longer_line">
- The purpose of this report is to look into the elements that influence students' grades. <br>
- The major findings were that students who study for longer periods of time are more likely to have a better ATAR score as well as satisfactory university academic achievement. Furthermore, it appears that a student's typical working hours and caffeine consumption are inversely proportional to their school performance. Finally, it appeared that public high school students had the widest spread of ATAR scores and the lowest median score.

 

## Initial Data Analysis (IDA)
- __source:__ Acquired from survey responses using volunteer selection <br>
- __sample:__ Consisting primarily of USYD students. <br>
- __variables__ 4 Qualitative (2 ordinal, 2 nominal), 6 Quantitative (All Discrete) <br>
- __limitation__: Data is based on self report, meaning that there are likely inaccuracies in quantifying hours of study, sleep etc. The assumption is made that the quantity reported by respondents is approximately accurate. It is also assumed that for questions with a more ‘objective’ type of answer, such as ATAR or type of school, the responses are honest and accurate. <br>
- __Data Wrangling__: For questions that required a quantitative response (such as number of hours or cups), any response containing a number and a word was cleaned to remove the word and record the number only. Data from respondents who gave consistently facetious responses was removed, as well as one response that did not provide their ATAR.

 
```{r, message=FALSE}
library(googlesheets4)
library(ggplot2)

surv_url <- "https://docs.google.com/spreadsheets/d/1JGMBBDOkDFzFrTvwRGJ43rXYGYUKbJXn6SZjeau4NRA/edit?resourcekey#gid=946826064"
surv_data <- read_sheet(surv_url)
rows <- dim(surv_data)[1]
columns <- dim(surv_data)[2] - 1
diag_size <- c(5,3)

v1 <- "(Qualitative)"
v2 <- "(Quantitative)"
var_ls <- c("Gender", "ATAR", "Study/week", "Sleep/day", "Work/week", "High School", "Academic Performance (self-rated)", "Physical Activity/week", "Tutoring/week (high school)", "Caffeine Consumption/week")

gender_M <- paste("Male =", length(surv_data[[2]][surv_data$Gender == "Male"]))
gender_F <- paste("Female =", length(surv_data$Gender[surv_data$Gender == "Female"]))
gender_N <- paste("Not Replied =", length(surv_data$Gender[surv_data$Gender == "Choose not to specify."]))

ATAR_mean <- paste("Mean =", round(mean(surv_data$ATAR), 2))
ATAR_median <- paste("Median =", median(surv_data$ATAR))

Study_mean <- paste("Mean =", round(mean(surv_data$Study), 2), "hr.")
Study_median <- paste("Median =", median(surv_data$Study), "hr.")

sleep_1 <- paste("4~6 hr. =", length(surv_data$Sleep[surv_data$Sleep == "4 to 6 hours"]))
sleep_2 <- paste("6~8 hr. =", length(surv_data$Sleep[surv_data$Sleep == "6 to 8 hours"]))
sleep_3 <- paste("8~10 hr. =", length(surv_data$Sleep[surv_data$Sleep == "8 to 10 hours"]))

Work_mean <- paste("Mean =", round(mean(surv_data$Work), 2), "hr.")
Work_median <- paste("Median =", median(surv_data$Work), "hr.")

Pub_school <- paste("Public=", length(surv_data$High_School[surv_data$High_School == "Public"]))
Selec_school <- paste("Selective=", length(surv_data$High_School[surv_data$High_School == "Selective"]))
Priv_school <- paste("Private=", length(surv_data$High_School[surv_data$High_School == "Private"]))

Academ_Rat_1 <- paste("1~4=", length(surv_data$Academic_Performance[surv_data$Academic_Performance %in% c(1:4)]))
Academ_Rat_2 <- paste("5~7=", length(surv_data$Academic_Performance[surv_data$Academic_Performance %in% c(5:7)]))
Academ_Rat_3 <- paste("8~10=", length(surv_data$Academic_Performance[surv_data$Academic_Performance %in% c(8:10)]))
Academ_Rat_mean <- paste("Mean =", round(mean(surv_data$Academic_Performance), 2))

Phys_mean <- paste("Mean =", round(mean(surv_data$Physical_Activity), 2), "hr.")
Phys_median <- paste("Median =", median(surv_data$Physical_Activity), "hr.")

Tut_mean <- paste("Mean =", round(mean(surv_data$Tutoring), 2), "hr.")
Tut_median <- paste("Median =", median(surv_data$Tutoring), "hr.")

Caf_mean <- paste("Mean =", round(mean(surv_data$Caffeine_Consumption), 2), "cups")
Caf_median <- paste("Median =", median(surv_data$Caffeine_Consumption), "cups")
```
Size of Data (# of reponses) = `r rows` <br>
<font size="1" color="navy">

|`r var_ls[1]`|`r var_ls[2]`|`r var_ls[3]`|`r var_ls[4]`|`r var_ls[5]`|`r var_ls[6]`|`r var_ls[7]`|`r var_ls[8]`|`r var_ls[9]`|`r var_ls[10]`|
|:----:|:----:|:----:|:----:|:----:|:----:|:----:|:----:|:----:|:----:|
| `r v1` | `r v2` | `r v2` | `r v1` |`r v2` | `r v1` | `r v1` | `r v2` |`r v2` |`r v2`|
|`r gender_M`|`r ATAR_mean`|`r Study_mean`|`r sleep_1` |`r Work_mean`|`r Pub_school` |`r Academ_Rat_1` | `r Phys_mean`|`r Tut_mean`|`r Caf_mean`|
|`r gender_F`|`r ATAR_median`|`r Study_median`| `r sleep_2` |`r Work_median`|`r Priv_school`| `r Academ_Rat_2`|`r Phys_median` |`r Tut_median`|`r Caf_median`|
|`r gender_N`| | |`r sleep_3` ||`r Selec_school`|`r Academ_Rat_3` |
||||||| `r Academ_Rat_mean` |

</font>
```{r}
surv_data$Gender <- factor(surv_data$Gender,
    levels=c("Male", "Female", "Choose noy to reply"))
library(datasets)
str(surv_data)
pairs(surv_data[c(2, 3, 4, 6, 8, 9, 10, 11)])
```


## Research Questions

### Q, What is the effect of Study time on student's academic performance
```{r, message=FALSE, fig.dim = diag_size}

SR_AP <- surv_data$Academic_Performance

ggplot(surv_data, aes(x=Study, y=ATAR)) +
  geom_point(aes(color=SR_AP ), size=3) + geom_smooth() +
  xlab("Average studying hours per week") + ylab("ATAR (High School)") +
   scale_colour_gradient(low = "yellow", high = "red") +
  labs(caption="SR_AP = Self-Rated Academic Performance")
```
```{r, message=FALSE, fig.dim = diag_size}
 ggplot(surv_data, aes(x=Study, y=Academic_Performance)) +
  geom_point(aes(color=ATAR), size=3) + geom_smooth() +
  xlab("Average studying hours per week") + ylab("Self-rated Academic Performance (Uni)") +
  scale_colour_gradient(low = "yellow", high = "green")

ac <- surv_data$Academic_Performance
divided_ac <- c()
for (i in 1:length(ac)){
  if (ac[i] > 0 & ac[i] <= 4){
    divided_ac <- append(divided_ac, "1~4")
  }
  else if (ac[i] > 4 & ac[i] <= 7){
    divided_ac <- append(divided_ac, "5~7")
  }
  else{
    divided_ac <- append(divided_ac, "8~10")
  }
}
names(surv_data$Academic_Performance) <- divided_ac
SR_AP <- names(surv_data$Academic_Performance)
ggplot(surv_data, aes(x=ATAR)) +
  geom_histogram(aes(fill=SR_AP), bins=5) +
  labs(x="ATAR", y="# of Students", caption="SR_AP = Self-Rated Academic Performance")

```
<font size="3"> <br> 
Can anyone describe the relationship of above two graphs using pearson's correlation value? or in a context of atar being standardised test and academic performance being self-rated. (on google doc) <br>
</font>

### Q. What is the effect of Environment on student's grade
```{r, message=FALSE, fig.dim = diag_size}
ggplot(surv_data, aes(x=High_School, y=ATAR, color=Gender)) +
  geom_boxplot() +
  xlab("Type of High School") + ylab("ATAR")
```
```{r, message=FALSE, fig.dim = diag_size}
ggplot(surv_data, aes(x=Tutoring, y=ATAR)) +
  geom_point() + geom_smooth() +
  xlab("Tutoring") + ylab("ATAR") 
```
<br>

### Q. What is the effect of Daily lifetyle on student's grade

```{r, message=FALSE, fig.dim = diag_size}
c <- surv_data$Caffeine_Consumption
drink_or_not <- c()
for (i in 1:length(c)){
  if (c[i] > 0 ){
    drink_or_not <- append(drink_or_not, "Caffeinated Drink O")
  }
  else{
    drink_or_not <- append(drink_or_not, "Caffeinated Drink X")
  }
}
names(surv_data$Caffeine_Consumption) <- drink_or_not

ggplot(surv_data, aes(x=names(Caffeine_Consumption), y=ATAR)) +
  geom_boxplot(aes(color=Sleep)) +
  labs(x="Caffeine Consumption (y/n) ", y="ATAR score")

divided_caffeine <- c()
for (i in 1:length(c)){
  if (c[i] == 0 ){
    divided_caffeine <- append(divided_caffeine, "0 cups")
  }
  else if (c[i] > 0 & c[i] <= 4){
    divided_caffeine <- append(divided_caffeine, "1~4 cups")
  }
  else{
    divided_caffeine <- append(divided_caffeine, "5+ cups")
  }
}
names(surv_data$Caffeine_Consumption) <- divided_caffeine

ggplot(surv_data, aes(x=names(Caffeine_Consumption), fill=Sleep)) + 
  geom_bar() +
  labs(x="caffeine consumption per week ", y="# of people (responses)")

ggplot(surv_data, aes(x=Work, y=Academic_Performance)) +
  geom_point() +geom_smooth() +
  labs(x="Weekly working hours", y="Uni Academic Performance")
```



## Articles

<br>

## References
Use APA

<br>

## Acknowledgments
When did you team meet (date and time), and what did each team member contribute?
